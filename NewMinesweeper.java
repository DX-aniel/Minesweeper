...
Resubmitted version of Minesweeper
Jan 19, 2020
...
public class NewMinesweeper extends JFrame implements ActionListener, MouseListener{
	...
	public void mousePressed( MouseEvent e){
		...
		if (SwingUtilities.isRightMouseButton(e)){ //handles flagging if user right clicks on button
			...
			if (flags[x][y] != FLAG && buttons[x][y].isEnabled()){ //if not flagged yet
			...
			else if (buttons[x][y].isEnabled()){ //remove flag if already flagged
				...
				*/
				for (int k = -1; k< 2; k++){
					...
					for (int l = -1; l<2; l++){
						...
						if (i-k <0 || i-k >SIDE-1 || j-l<0 || j-l >SIDE -1 ){
							continue;
						}
